...
if __name__ == '__main__':
    ...
    for i in range(SIZE):
        ...
        print_log(
            "Current class avg: %f" % (total_per_cat_acc[current_cls_name] / total_per_cat_seen[current_cls_name]))
        print_log(
            "Current class iou: %f" % (total_per_cat_iou[current_cls_name] / total_per_cat_seen[current_cls_name]))
        ...
        output_color_point_cloud(ps, segp == seg, './test_results/diff_%d.obj' % (i),
                                 lambda eq: (0, 1, 0) if eq else (1, 0, 0))
